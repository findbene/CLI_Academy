  ...
  return (
    <article className="panel panel-lift p-6">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {course.layer ? <span className="badge">{course.layer}</span> : null}
        {course.badge ? <span className="badge" data-tone="warning">{course.badge}</span> : null}
      </div>
      ...
      {course.specialModule ? (
        <div className="mt-4 rounded-[var(--radius-lg)] border border-[var(--color-border-subtle)] bg-[var(--color-accent-subtle)] p-4 text-sm">
          <div className="font-semibold text-[var(--color-fg-default)]">Special module</div>
          <div className="mt-1 text-[var(--color-fg-muted)]">{course.specialModule}</div>
        </div>
      ) : null}
      <div className="mt-4">
      ...
      {course.modeSupport?.length ? (
        <div className="mt-4">
          <div className="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--color-fg-muted)]">Learning modes</div>
          <div className="mt-2 flex flex-wrap gap-2">
            {course.modeSupport.map((mode) => (
              <span key={mode} className="badge">{mode}</span>
            ))}
          </div>
        </div>
      ) : null}
      {course.mediaKit?.length ? (
        <div className="mt-4">
          <div className="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--color-fg-muted)]">Rich media standard</div>
          <div className="mt-2 flex flex-wrap gap-2">
            {course.mediaKit.map((item) => (
