...
const fsSource = `
  ...
  const vec4 bgColor1 = vec4(0.04, 0.05, 0.08, 1.0);
  const vec4 bgColor2 = vec4(0.02, 0.03, 0.05, 1.0);
  ...
  void main() {
    ...
    for(int l = 0; l < linesPerGroup; l++) {
      ...
      vec4 col1 = vec4(0.000, 0.400, 0.900, 1.0);
      vec4 col2 = vec4(0.000, 0.600, 1.000, 1.0);
      vec4 col3 = vec4(0.000, 0.800, 1.000, 1.0);
      vec4 dynamicColor = mix(col1, col2, sin(tColor * 6.28) * 0.5 + 0.5);
      dynamicColor = mix(dynamicColor, col3, cos(tColor * 6.28) * 0.5 + 0.5);

      lines += line * dynamicColor * (rand + 0.4);
    }
...
const ShaderBackground = ({ className }: ShaderBackgroundProps) => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Handle null return from getUniformLocation
